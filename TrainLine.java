...
public void shuffleLine() {
	...
	this.leftTerminus.setRight(lineMap[1]);

	TrainStation index = lineMap[1];
	for(int i=1; i<lineMap.length-1; i++) {
		index.setNonTerminal();
		index.setRight(lineMap[i+1]);
		index.setLeft(lineMap[i-1]);
	...
	this.rightTerminus.setLeft(lineMap[lineMap.length-2]);
